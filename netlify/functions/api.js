...
exports.handler = async (event) => {
  ...
  try {
    ...
    // GET: Fetch the queue
    if (method === 'GET') {
      const [rows] = await connection.execute('SELECT * FROM antrian ORDER BY priority DESC, tanggalDaftar ASC');
    ...
    if (method === 'POST') {
      ...
      if (body.action === 'add') {
        await connection.execute(
          ...
          [Date.now(), body.youtubeName, body.memberType, body.layanan, body.discord, body.keterangan, 'menunggu', body.priority, new Date()]
        );
      } else if (body.action === 'updateStatus') {
        await connection.execute('UPDATE antrian SET status = ? WHERE id = ?', [body.status, body.id]);
      }
